...
path_cost=0;
goal_distance=distance(xNode,yNode,xTarget,yTarget);%dist=sqrt((x1-x2)^2 + (y1-y2)^2);这里的距离是点到点最近的距离                                                                                    %%%  ***调用distance（）函数，求两坐标点之间的笛卡尔距离
% Sgoal=goal_distance;
...
while((xNode ~= xTarget || yNode ~= yTarget) && NoPath == 1)       %%%  判断当前点是否等于目标点
 ...
 for i=1:exp_count         %%% 把exp_array内的元素添加到 开启列表 里面
    flag=0;                %%% 将exp_array内的点的标志位设为0,flag=1代表存在相同数组
...
%    p_yNode=OPEN(index_min_node,5);
  ...
  CLOSED(CLOSED_COUNT,2)=yNode;
  OPEN(index_min_node,1)=0;%将新的子节点标记为0（已经经过检测）
